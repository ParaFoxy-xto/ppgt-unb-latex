...
:description: Modelo LaTeX para elaboração de dissertações e teses do Programa de Pós-Graduação em Transportes (PPGT) da Universidade de Brasília (UnB)

= {description}

== Introdução

Este é o modelo (template) LaTeX **OFICIAL** para elaboração de dissertações de mestrado e teses de doutorado do http://www.ppgt.unb.br[Programa de Pós-Graduação em Transportes (PPGT)] do Departamento de Engenharia Civil e Ambiental da http://ftd.unb.br[Faculdade de Tecnologia (FT)] da http://unb.br[UnB].

Este template foi adaptado do modelo da FT-UnB para atender às especificações de formatação do PPGT, incluindo:

- Formatação de capítulos centralizada ("1 INTRODUÇÃO")
- Tamanhos de fonte específicos (14pt para capítulos, 12pt para seções)
- Espaçamento 1,5 linhas sem recuo de parágrafo
- Lista de Quadros (além de Tabelas e Figuras)
- Folha de aprovação no formato PPGT
- Ficha catalográfica ajustada
- Bibliografia no formato ABNT alfabético

IMPORTANT: Este template deve ser usado para todos os trabalhos acadêmicos do PPGT.
